...
<svg id="kiddolens_logo" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 328.12 238.24">
  ...
  <!-- Blue Shape (Hook F1 -> Bar F2 -> Hook F3) -->
  <!-- We place blue first because in F3, the yellow petal needs to sit on top of the blue hook -->
  <path class="blue-shape" d="${hookF1}">
     <animate attributeName="d" dur="${duration}" repeatCount="1" fill="freeze" calcMode="spline" keySplines="0.4 0 0.2 1; 0.4 0 0.2 1" keyTimes="0; 0.5; 1"
      values="${hookF1};
              ${hookF2};
              ${hookF3}"
     />
  ...
  <!-- Yellow Shape (Right Petal F1 -> Bar F2 -> Left Petal F3) -->
  <path class="yellow-shape" d="${petalF1}">
     <animate attributeName="d" dur="${duration}" repeatCount="1" fill="freeze" calcMode="spline" keySplines="0.4 0 0.2 1; 0.4 0 0.2 1" keyTimes="0; 0.5; 1"
      values="${petalF1};
              ${petalF2};
              ${petalF3}"
...
fs.writeFileSync('kiddolens_logo_animated.svg', svgCode); // For preview reference
